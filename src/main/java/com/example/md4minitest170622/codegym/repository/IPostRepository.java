...
public interface IPostRepository extends PagingAndSortingRepository<Post, Long> {
    ...
    @Query(value = "select * from posts p\n" +
            "order by p.id desc limit 4 order by YEAR(p.create_at) asc", nativeQuery = true)
    Iterable<Post> showFourNewPost();

    @Query(value = "select * from posts p where p.title like ? and YEAR(p.create_at) = ? order by YEAR(p.create_at) asc", nativeQuery = true)
    Iterable<Post> searchByTitleAndYear(String title, int year);

    @Query(value = "select * from posts p where p.title like ? order by YEAR(p.create_at) asc", nativeQuery = true)
